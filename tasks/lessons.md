...
## 2026-02-22 (Storage Migration Scope Correction)
- Date: 2026-02-22
- Context: User clarified the app is still in development with no external installs, so migration code for legacy on-disk files is unnecessary.
- Mistake pattern: Implemented and retained migration/legacy compatibility paths without validating whether any real upgrade path existed yet.
- Prevention rule: Before adding migration scaffolding, explicitly confirm there is deployed user data that requires upgrade compatibility; if not, prefer a clean current-schema implementation and defer migration logic until release timing demands it.
- Verification: Removed `StorageMigrationManager`, migration marker/directory handling, and legacy decode branches; reran `swift test` and `xcodebuild`.
