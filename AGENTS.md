...
## Maintenance Update (2026-02-22, Visual 'Glass' Redesign)
- What changed:
  - Introduced `VisualEffectView` (`NSViewRepresentable` around `NSVisualEffectView`) to support AppKit materials in SwiftUI.
  - Applied `.underWindowBackground` to `RootView` background and `.contentBackground` to `TextEditor` instances in `PromptEditorView` and `OutputView`.
  - Added progressive disclosure: `OutputView` is now conditionally hidden on launch and transitions in `.move(edge: .bottom).combined(with: .opacity)` only after a prompt generation is triggered.
  - Added accessibility support: respects `\.accessibilityReduceTransparency` environment value to gracefully fall back to `NSColor.windowBackgroundColor`.
- Why it changed:
  - Refresh the visual style to a more native Apple "Liquid Glass" aesthetic while reducing cognitive load on idle app open, per redesign requirements.
- How it was verified:
  - UI successfully compiles.
  - `swift test` (80 tests passed).
- Durable caveats:
  - UI layouts rely heavily on `VisualEffectView`; ensure any new overlaying components have appropriate `blendingMode` contexts to avoid visual clipping or opaque blocking.
