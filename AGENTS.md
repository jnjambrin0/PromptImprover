...
- Confirm streaming output updates while running.
...
- Known pitfall (Codex discovery): when Codex is installed via `nvm`, GUI app PATH may miss `nvm` bin paths; fallback now scans versioned `nvm` directories as mitigation.
...
- Additional context for discovery failures: app/Xcode runtime may not include `nvm` paths in PATH because `nvm` is often loaded in `~/.zshrc` (interactive), while non-interactive shell discovery and GUI app environments may not source it.
