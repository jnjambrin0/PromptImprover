...
final class GuideDocumentManager: GuideDocumentManaging {
    ...
    func data(for guide: GuideDoc) throws -> Data {
        if guide.isBuiltIn {
            ...
            let builtInStoragePath = canonicalBuiltInStoragePath(for: guide)
            let builtInURL = try absoluteURL(forStoragePath: builtInStoragePath)
            if !fileManager.fileExists(atPath: builtInURL.path) {
                try materializeBuiltInGuide(guideID: guide.id, storagePath: builtInStoragePath, destinationURL: builtInURL)
            }

            guard fileManager.fileExists(atPath: builtInURL.path) else {
                throw PromptImproverError.guideManagementFailed("Missing built-in guide document at \(builtInStoragePath).")
            }

            return try Data(contentsOf: builtInURL)
        }
    ...
    private func canonicalBuiltInStoragePath(for guide: GuideDoc) -> String {
        let normalized = GuideDoc.normalizeStoragePath(guide.storagePath) ?? guide.storagePath
        if normalized.lowercased().hasPrefix("guides/builtin/") {
            return normalized
        }

        return "guides/builtin/\(URL(fileURLWithPath: normalized).lastPathComponent)"
    }

    private func materializeBuiltInGuide(guideID: String, storagePath: String, destinationURL: URL) throws {
        let templateRelativePath = GuidesDefaults.builtInTemplateRelativePath(forGuideID: guideID)
            ?? URL(fileURLWithPath: storagePath).lastPathComponent
        let builtInData = try templates.data(forRelativePath: templateRelativePath)
        try atomicFileStore.write(builtInData, to: destinationURL)
    }
}
