...
final class GuideDocumentManager: GuideDocumentManaging {
    ...
    func ensureEditableGuide(_ guide: GuideDoc) throws -> GuideDoc {
        ...
        let forkURL = try userGuideAbsoluteURL(forStoragePath: forkStoragePath)
        let builtInData = try templates.data(forRelativePath: guide.storagePath)
    ...
    func saveText(_ text: String, for guide: GuideDoc) throws -> GuideDoc {
        ...
        if guide.isBuiltIn {
            ...
            let userURL = try userGuideAbsoluteURL(forStoragePath: updatedGuide.storagePath)
