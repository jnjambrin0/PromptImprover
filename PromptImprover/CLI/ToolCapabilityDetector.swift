...
struct ToolCapabilityDetector: ToolCapabilityDetecting {
    ...
    private func detectClaudeCapabilities(supportsModelFlag: Bool, lowerHelp: String) -> ToolCapabilities {
        ...
        let parsedValues = ToolCapabilityDetector.extractEffortValues(from: lowerHelp)
        let supportedValues = parsedValues.isEmpty ? EngineEffort.allCases : parsedValues
    ...
    private func detectCodexCapabilities(
        ...
        if hasExplicitEffortSignal {
            return ToolCapabilities(
                ...
                supportedEffortValues: parsedValues.isEmpty ? EngineEffort.allCases : parsedValues
            )
        }
    ...
    private static func extractEffortValues(from text: String) -> [EngineEffort] {
        var ordered: [EngineEffort] = []
        for effort in EngineEffort.allCases {
