    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                .disabled(launchAtLoginManager.status == .unavailable)

                if let startupMessage = launchAtLoginManager.message {
                    Text(startupMessage)
                        .font(.caption)
                ...
                if let startupError = launchAtLoginManager.errorMessage {
                    Text(startupError)
                        .font(.caption)
                        .foregroundStyle(.red)
                }
...
#Preview {
    UpdatesSettingsView(
        ...
        launchAtLoginManager: LaunchAtLoginManager(service: UpdatesPreviewLaunchAtLoginService())
    )
...
@MainActor
private final class UpdatesPreviewLaunchAtLoginService: LaunchAtLoginServiceControlling {
    var status: LaunchAtLoginServiceStatus = .requiresApproval
