...
struct EngineSettingsStoreTests {
    ...
    func writesSchemaVersionAsInteger() throws {
        ...
        #expect(document["schemaVersion"] as? Int == EngineSettingsStore.currentSchemaVersion)
    }

    @Test
    func fallsBackToDefaultsWhenFileMissingOrCorrupt() throws {
        let missingFileURL = makeSettingsFileURL()
        let missingStore = EngineSettingsStore(fileURL: missingFileURL)
        #expect(missingStore.load() == .default)

        let corruptFileURL = makeSettingsFileURL()
        try Data("not-json".utf8).write(to: corruptFileURL, options: .atomic)
        let corruptStore = EngineSettingsStore(fileURL: corruptFileURL)
        #expect(corruptStore.load() == .default)
    }
    ...
    @Test
    func fallsBackToDefaultsWithoutSchemaVersion() throws {
        let fileURL = makeSettingsFileURL()
        let json = """
        {
          "settingsByTool": {
