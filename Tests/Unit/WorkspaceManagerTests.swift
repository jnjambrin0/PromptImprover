...
struct WorkspaceManagerTests {
    @Test
    func codexWorkspaceContainsExpectedRuntimeFilesAndTemplates() throws {
        ...
        #expect(!FileManager.default.fileExists(atPath: workspace.path.appendingPathComponent("TARGET_MODEL.txt").path))
    }

    @Test
    func claudeWorkspaceContainsExpectedRuntimeFilesAndTemplates() throws {
        let templates = Templates(bundle: .main, fallbackRoot: templateRootURL())
        let manager = WorkspaceManager(templates: templates)

        let request = makeRequest(tool: .claude, mappedGuides: [])
        let workspace = try manager.createRunWorkspace(request: request)
        defer { workspace.cleanup() }
    ...
    func workspaceCopiesOnlyMappedGuidesInOrderAndWritesRunConfig() throws {
        ...
        #expect(workspace.guideFilenamesInOrder == expected)
    ...
    @Test
    func workspaceNormalizesInputPromptLineEndingsAndOuterWhitespace() throws {
        let templates = Templates(bundle: .main, fallbackRoot: templateRootURL())
        let manager = WorkspaceManager(templates: templates)
        ...
        let persistedInput = try String(contentsOf: workspace.inputPromptPath, encoding: .utf8)
        #expect(persistedInput == "line1\nline2")
    }
    ...
    func workspaceDoesNotWriteEffortLevelForNonClaudeOrMissingEffort() throws {
        ...
        #expect(!FileManager.default.fileExists(atPath: codexSettingsPath.path))
