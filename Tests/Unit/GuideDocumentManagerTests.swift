...
struct GuideDocumentManagerTests {
    ...
    @Test
    func resolvesBuiltInGuidesFromBundleTemplatesEvenWhenAppSupportCopyIsStale() throws {
        let context = try makeContext()
        let manager = context.makeManager(maxImportSizeBytes: 1_048_576)
        let guide = try #require(GuidesCatalog.default.guide(id: GuidesDefaults.gptGuideID))

        let staleCopyURL = context.appSupportDirectory.appendingPathComponent(guide.storagePath)
        try FileManager.default.createDirectory(at: staleCopyURL.deletingLastPathComponent(), withIntermediateDirectories: true)
        try Data("STALE_BUILTIN_COPY".utf8).write(to: staleCopyURL, options: .atomic)

        let templateRelativePath = try #require(GuidesDefaults.builtInTemplateRelativePath(forGuideID: guide.id))
        let expectedData = try Templates(bundle: .main, fallbackRoot: templateRootURL()).data(forRelativePath: templateRelativePath)
        let resolvedData = try manager.data(for: guide)

        #expect(resolvedData == expectedData)
        #expect(FileManager.default.fileExists(atPath: staleCopyURL.path))
    }

    @Test
    func builtInGuideMappingsResolveToExistingTemplateFiles() throws {
        let templates = Templates(bundle: .main, fallbackRoot: templateRootURL())
        for builtInGuide in GuidesDefaults.builtInGuides {
            let templateRelativePath = try #require(GuidesDefaults.builtInTemplateRelativePath(forGuideID: builtInGuide.id))
            let data = try templates.data(forRelativePath: templateRelativePath)
            #expect(!data.isEmpty)
        }
    }
